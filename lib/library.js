...
import { STARTER_CHORDS } from '../data/starter.js';
...
// Records that the starter chord has already been offered. Kept separate from
// the library itself so that deleting the starter is permanent: an empty
// library and a never-seeded one have to be distinguishable, and `fct_chords`
// alone cannot tell them apart.
export const SEEDED_KEY = PREFIX + 'seeded';
...
//
// On the very first run this also seeds the starter chord, so the app opens
// with something in it. It is written to storage immediately and owned by the
// user from then on — nothing re-seeds, and deleting it makes it stay gone.
export function loadChords() {
  ...
  if (raw === null && !hasSeeded()) {
    markSeeded();
    const starter = STARTER_CHORDS.filter(isUsable);
    saveChords(starter);
    return { chords: starter, dropped: 0 };
  }
...
function hasSeeded() {
  try { return localStorage.getItem(SEEDED_KEY) !== null; } catch (e) { return true; }
}
function markSeeded() {
  try { localStorage.setItem(SEEDED_KEY, JSON.stringify(true)); } catch (e) {}
}
